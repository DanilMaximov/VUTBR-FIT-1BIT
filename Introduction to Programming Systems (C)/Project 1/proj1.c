...
#define MAX_TEMPLATES_COUNT                                                    \
  10000             // max number of combinations for name searching
...
void generate_templates(char *template, char *str, int index,
  ...
  int buttons[26] = {2, 2, 2, 3, 3, 3, 4, 4, 4, 5, 5, 5, 6,
...
void printf_result(char *argv[], list contact[], int counter_for_name,
                   int counter_for_number) {
  ...
  for (int i = 0; i < counter_for_name; i++) {
    ...
    if ((contact[i].name_mathes == contact[i].number_mathes) &&
        contact[i].number_mathes > 0 && contact[i].name_mathes > 0)
      contact[i].number_mathes++; // priority by number with the same number of
                                  // matches
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 1) {
    ...
    int check =
        input_check(argv[1]); // checking the string for the contents of numbers
